...
function hexToRgb(hex) {
    ...
    if (hex.length == 3) {
        red = hexToDec(hex.substring(0, 1) + hex.substring(0, 1));
        green = hexToDec(hex.substring(1, 1) + hex.substring(1, 1));
        blue = hexToDec(hex.substring(2, 1) + hex.substring(2, 1));
    } else {
        red = hexToDec(hex.substring(0, 2));
...
function mostBrightColor(colors) {
    var mostBright = false;
    var color, hex, brightness;

    for (i = 0; i < colors.length; i++) {
        color = hexValueSanitize(colors[i]);
        hex = color.replace('#', '');

        brightness = hexBrightness(hex);
        if (!mostBright || hexBrightness(hex) > hexBrightness(mostBright)) {
            mostBright = hex;
        }
...
function brightestDullColor(colors) {
    ...
    for (i = 0; i < colors.length; i++) {
        ...
        if (!brightestDull || hexBrightness(hex) * howDull > hexBrightness(brightestDull) * howDullOld) {
            brightestDull = hex;
        }
    }
...
function mix_colors(hex1, hex2, percent) {
    ...
    r2 = hexToDec(hex2.substr(0, 2));
    g2 = hexToDec(hex2.substr(2, 2));
    b2 = hexToDec(hex2.substr(4, 2));

    red = (percent * r1 + (100 - percent) * r2) / 100;
    green = (percent * g1 + (100 - percent) * g2) / 100;
    blue = (percent * b1 + (100 - percent) * b2) / 100;

    var red_hex = strPad(decToHex(red), 2, '0', 'STR_PAD_LEFT');
    var green_hex = strPad(decToHex(green), 2, '0', 'STR_PAD_LEFT');
    var blue_hex = strPad(decToHex(blue), 2, '0', 'STR_PAD_LEFT');
...
lineReader.on('line', function(line) {
    ...
    a = line.split(",");
    h1 = mostBrightColor(a);
    a.splice(a.indexOf(h1), 1);
    h2 = mostBrightColor(a);
    a.splice(a.indexOf(h2), 1);
    fill = mostBrightColor(a);
    a.splice(a.indexOf(fill), 1);
    s1 = mostBrightColor(a);
    a.splice(a.indexOf(s1), 1);
    s2 = mostBrightColor(a);
